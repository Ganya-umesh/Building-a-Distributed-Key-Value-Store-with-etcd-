...
def main():
    st.sidebar.title("Leader Details")
    ...
    for port in ports:
        try:
            ...
            st.sidebar.text(f"Leader: {stat.leader}")
            st.sidebar.text(f"etcd version: {stat.version}")
            break
        
        except Exception as e:
            st.error(f"Node with port {port} is not reachable. Error: {e}")        
    
    st.title("Welcome to etcd Cluster Operations")
    st.markdown(
        """
        <style>
        body {
